...
public class AILerp : MonoBehaviour {
	...
	protected virtual void Update () {
		if (canMove) {
            ...
            Vector3 nextPos = CalculateNextPosition(out curDirection);

			// Rotate unless we are really close to the target
            if (enableRotation && curDirection != Vector3.zero)
            {
				if (rotationIn2D) {
                    float angle = Mathf.Atan2(curDirection.x, -curDirection.y) * Mathf.Rad2Deg + 180;
					Vector3 euler = tr.eulerAngles;
					euler.z = Mathf.LerpAngle(euler.z, angle, Time.deltaTime * rotationSpeed);
					tr.eulerAngles = euler;
				} else {
					Quaternion rot = tr.rotation;
                    Quaternion desiredRot = Quaternion.LookRotation(curDirection);
